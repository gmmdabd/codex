    ...
    checkAuth().then(user => {
        ...
        const homeHTML = `
            <div class="navbar">
                ...
                <ul class="nav-links">
                    <li><a href="#" id="login-link" aria-label="Login"><i class="fas fa-sign-in-alt"></i> <span class="nav-text">Login</span></a></li>
                    <li><a href="#" id="signup-link" aria-label="Sign Up"><i class="fas fa-user-plus"></i> <span class="nav-text">Sign Up</span></a></li>
                </ul>
            </div>
            <div class="container">
                <div class="hero-section">
                    <div class="hero-content">
                        ...
                        <div class="hero-buttons">
                            <button class="btn" id="get-started"><i class="fas fa-rocket"></i> <span class="nav-text">Get Started</span></button>
                            <button class="btn btn-outline" id="learn-more"><i class="fas fa-info-circle"></i> <span class="nav-text">Learn More</span></button>
                        </div>
                    </div>
    ...
    checkAuth().then(async user => {
        ...
        const dashboardHTML = `
            <div class="navbar">
                <div class="logo"><i class="fas fa-book-open"></i> urSchoolNotez</div>
                <ul class="nav-links">
                    <li><a href="#" id="dashboard-link" class="active" aria-label="Dashboard"><i class="fas fa-home"></i> <span class="nav-text">Dashboard</span></a></li>
                    <li><a href="#" id="community-link" aria-label="Community"><i class="fas fa-users"></i> <span class="nav-text">Community</span></a></li>
                    <li><a href="#" id="logout-link" aria-label="Logout"><i class="fas fa-sign-out-alt"></i> <span class="nav-text">Logout</span></a></li>
                </ul>
            </div>
...
async function renderSectionPage(classNum, section) {
    checkAuth().then(async user => {
        ...
        currentUser = user;
        const userSchool = user.user_metadata.school;
        
        // Fetch notes for this section from Supabase - only for this class and school
        ...
        const sectionHTML = `
            <div class="navbar">
                <div class="logo"><i class="fas fa-book-open"></i> urSchoolNotez</div>
                <ul class="nav-links">
                    <li><a href="#" id="dashboard-link" aria-label="Dashboard"><i class="fas fa-home"></i> <span class="nav-text">Dashboard</span></a></li>
                    <li><a href="#" id="community-link" aria-label="Community"><i class="fas fa-users"></i> <span class="nav-text">Community</span></a></li>
                    <li><a href="#" id="logout-link" aria-label="Logout"><i class="fas fa-sign-out-alt"></i> <span class="nav-text">Logout</span></a></li>
                </ul>
            </div>
            <div class="container">
                <div class="notes-header">
                    ...
                    <p class="school-indicator">${userSchool}</p>
                    <button class="btn" id="upload-note" aria-label="Upload Note"><i class="fas fa-upload"></i> <span class="nav-text">Upload New Note</span></button>
                </div>
                ...
                <div class="search-bar">
                    ...
                    <div class="search-buttons" style="display: flex; gap: 10px; ${isMobile ? 'width: 100%;' : ''}">
                        <button class="btn" id="search-btn" style="${isMobile ? 'flex: 1;' : ''}"><i class="fas fa-search"></i> <span class="nav-text">Search</span></button>
                        <button class="btn btn-outline" id="reset-search" style="${isMobile ? 'flex: 1;' : ''}"><i class="fas fa-redo"></i> <span class="nav-text">Reset</span></button>
                    </div>
                </div>
                    ...
                    ${notes && notes.length > 0 ? notes.map(note => `
                        <div class="note-card">
                            ...
                            <div class="note-body">
                                ...
                                <a href="${note.file_url}" target="_blank" class="btn"><i class="fas fa-eye"></i> <span class="nav-text">View Note</span></a>
                            </div>
...
function renderUploadPage() {
    checkAuth().then(async user => {
        ...
        const uploadHTML = `
            ...
            <div class="container">
                <div class="upload-container">
                    ...
                    <p class="upload-subtitle">For ${userSchool} - Class ${user.user_metadata.class} - Section ${currentSection}</p>
                    <form id="upload-form">
                        ...
                        <div class="form-group">
                            ...
                            ${isMobile ? `
                                <button type="button" class="btn btn-block mobile-upload-btn" id="mobile-upload-btn">
                                    <i class="fas fa-file-upload"></i> Select File
                                </button>
                                <input type="file" id="file-input" class="file-input" accept=".pdf,.jpg,.jpeg,.png" required style="display: none;">
                            ` : `
                        ...
                        <button type="submit" class="btn btn-block" id="upload-submit-btn" ${isMobile ? 'disabled' : ''}>
                            <i class="fas fa-upload"></i> <span class="nav-text">Upload Note</span>
                        </button>
                    </form>
                </div>
            </div>
        ...
        if (isMobile) {
            ...
            fileInput.addEventListener('change', (e) => {
                if (fileInput.files.length > 0) {
                    ...
                    mobileUploadBtn.innerHTML = `<i class="fas fa-check"></i> File Selected`;
                } else {
                    fileName.textContent = '';
                    fileName.classList.remove('file-selected');
                    uploadSubmitBtn.disabled = true;
                    mobileUploadBtn.innerHTML = `<i class="fas fa-file-upload"></i> Select File`;
                }
            });
        } else {
        ...
        // Regular form submission for desktop
        uploadForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            await submitUpload(user);
        });
        
        // Function to handle upload submission
        async function submitUpload(user) {
            ...
            // Validate
            if (!file) {
                errorElem.innerHTML = '<i class="fas fa-exclamation-circle"></i> Please select a file to upload';
            ...
            if (!noteTitle || noteTitle.trim() === '') {
                errorElem.innerHTML = '<i class="fas fa-exclamation-circle"></i> Please enter a title for your note';
                errorElem.classList.remove('hidden');
                successElem.classList.add('hidden');
                return;
            }
            ...
            try {
                ...
                submitBtn.innerHTML = '<i class="fas fa-upload"></i> <span class="nav-text">Upload Note</span>';
        ...
        if (isMobile) {
            // Prevent double-tap zoom on buttons
            document.querySelectorAll('button:not(.btn), a').forEach(element => {
                element.addEventListener('touchend', (e) => {
                    if (element.tagName === 'A' && element.getAttribute('href') !== '#') {
                        return; // Allow normal behavior for links with real URLs
                    }
...
function renderCommunityPage() {
    checkAuth().then(async user => {
        ...
        // Fetch community posts from Supabase - only for this class and school
        const { data: classPosts, error } = await supabase
        ...
        const communityHTML = `
            <div class="navbar">
                ...
                <ul class="nav-links">
                    <li><a href="#" id="dashboard-link" aria-label="Dashboard"><i class="fas fa-home"></i> <span class="nav-text">Dashboard</span></a></li>
                    <li><a href="#" id="community-link" class="active" aria-label="Community"><i class="fas fa-users"></i> <span class="nav-text">Community</span></a></li>
                    <li><a href="#" id="logout-link" aria-label="Logout"><i class="fas fa-sign-out-alt"></i> <span class="nav-text">Logout</span></a></li>
                </ul>
            </div>
            <div class="container">
                <div class="community-container">
                    <div class="welcome-header">
                        <h1><i class="fas fa-users"></i> Class ${userClass} Community</h1>
                        <p>Connect with your classmates at ${userSchool}</p>
                    </div>
                    ...
                    <div class="post-form">
                        ...
                        <form id="post-form">
                            ...
                            <button type="submit" class="btn" id="post-submit-btn"><i class="fas fa-paper-plane"></i> <span class="nav-text">Post</span></button>
                        </form>
                    </div>
                    
                    <div class="posts-container">
                        <h3 class="section-header">Class ${userClass} Posts at ${userSchool}</h3>
                        ${classPosts && classPosts.length > 0 ? classPosts.map(post => {
                                    ...
                                    <div class="post-replies" id="replies-${post.id}" style="display: none;">
                                        ...
                                        <div class="reply-form" id="reply-form-${post.id}">
                                            <form class="post-reply-form" data-post-id="${post.id}">
                                                ...
                                                <button type="submit" class="btn reply-submit-btn"><i class="fas fa-paper-plane"></i> <span class="nav-text">Submit Reply</span></button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                            `;
                        }).join('') : '<div class="empty-state"><i class="fas fa-comments"></i><p>No posts in your class yet. Be the first to post!</p></div>'}
                    </div>
                </div>
            </div>
        `;
        ...
        if (postForm) {
            // Fix for mobile: Add click event to the submit button
            if (isMobile && postSubmitBtn) {
                postSubmitBtn.addEventListener('click', async (e) => {
                    e.preventDefault();
                    await submitPost(user, postSubmitBtn);
                });
            }
        ...
        async function submitPost(user, submitBtn) {
        ...
        replyForms.forEach(form => {
            const replySubmitBtn = form.querySelector('.reply-submit-btn');
            const postId = form.getAttribute('data-post-id');
            
            // Fix for mobile: Add click event to the submit button
            if (isMobile && replySubmitBtn) {
                replySubmitBtn.addEventListener('click', async (e) => {
                    e.preventDefault();
                    await submitReply(user, form, replySubmitBtn, postId);
                });
            }
        ...
        async function submitReply(user, form, submitBtn, postId) {
